...
namespace CinemAPI.Data.Implementation
{
    public class ProjectionRepository : IProjectionRepository
    {
        ...
        public int GetAvailableSeatsForProjection(long projectionId)
        {

            return db.Projections.FirstOrDefault((p) => p.Id == projectionId).AvailableSeatsCount;
        }
        ...
        public bool ProjectionExists(long projectionId)
        {
            return db.Projections.FirstOrDefault((p) => p.Id == projectionId) != null;
        }

        public bool IsProjectionActive(long projectionId)
        {
            DateTime now = DateTime.UtcNow;

            return db.Projections.FirstOrDefault((p) => p.Id == projectionId).StartDate > now;
        }
    }
}